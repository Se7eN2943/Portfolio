...
export default class Task extends Component {
  state = {
    timestamp: formatDistanceToNow(Date.now(), {
      addSuffis: false,
  ...
  componentDidMount = async () => {
    ...
    this.interval = setInterval(() => {
      this.setState({
        timestamp: formatDistanceToNow(timestamp, { includeSeconds: true })
      })
  ...
  timer = (event) => {
    ...
    if (!event) return
    if (
      (event === 'continue' || event.target.name === 'play') &&
      !checked &&
      time !== 0
    ) {
      ...
      <li className={checkClass}>
        <div className="view">
          ...
          <label htmlFor={id}>
            ...
            <span className="description">
              created {this.state.timestamp} ago
            </span>
          </label>
